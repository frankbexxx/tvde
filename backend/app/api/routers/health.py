...
async def config() -> dict[str, str | bool]:
    """Public config for frontend (BETA flag + Google OAuth readiness).

    ``google_oauth_enabled`` depends only on ``GOOGLE_OAUTH_*`` credentials —
    independent of ``BETA_MODE`` (B1 exchange already was; this matches the UI).
    """
