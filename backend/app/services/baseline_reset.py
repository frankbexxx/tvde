...
# Shared demo password is for passenger/driver test logins only.
# Privileged baseline roles must never receive TEST_ACCOUNT_PASSWORD.
...
def _baseline_role_for_phone(phone: str) -> Role | None:
    normalized = phone.strip()
    for baseline_phone, role, _ in BASELINE_USERS:
        if baseline_phone == normalized:
            return role
    return None
...
def _is_shared_test_seed_account(phone: str, role: Role | None) -> bool:
    if settings.is_owner_phone(phone):
        return False
    resolved_role = role if role is not None else _baseline_role_for_phone(phone)
    if resolved_role is None:
        # Non-baseline phones used by dev seed helpers are passenger/driver demos.
        return True
    return resolved_role in _SHARED_TEST_ACCOUNT_ROLES
