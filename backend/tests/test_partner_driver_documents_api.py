...
def test_driver_reupload_resets_approved_document_to_pending_review() -> None:
    ...
    try:
        pid = uuid.uuid4()
        db.add(Partner(id=pid, name="Doc Reupload Status"))
        u_d = User(
            role=Role.driver,
            name="Reupload Status Driver",
            phone=f"+3519{uuid.uuid4().int % 10_000_000:07d}",
            status=UserStatus.active,
        )
        u_p = User(
            role=Role.partner,
            name="Reupload Status Partner",
            phone=f"+3519{uuid.uuid4().int % 10_000_000:07d}",
            status=UserStatus.active,
            partner_org_id=pid,
        )
    ...
    r_up = c.post(
        "/driver/documents/carta_tvde/upload",
        headers={"Authorization": f"Bearer {driver_tok}"},
        files={"file": ("licenca.pdf", b"%PDF-1.4 original", "application/pdf")},
    )
    assert r_up.status_code == 200
    ...
    doc = r_reupload.json()["docs"]["carta_tvde"]
    assert doc["status"] == "pending_review"
    assert doc["file_name"] == "nova-licenca.pdf"
