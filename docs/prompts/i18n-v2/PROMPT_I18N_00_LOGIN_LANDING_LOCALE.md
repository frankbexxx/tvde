# E0 — i18n login-landing-locale

**ID:** `E0` · **Branch:** `feat/i18n-e0-login-landing-locale` · **Depende de:** #353 em main
...
git checkout -b feat/i18n-e0-login-landing-locale
```

## Objectivo

PT | EN **antes** de entrar: toggle no login e landing `/download`; migrar `(beta mode)`, hint telemóvel, Google, footer versão; actualizar I18N.md; estender `locale-en-smoke` com toggle no login (não só `addInitScript`).

## Ficheiros (âmbito)

- [`features/auth/LoginScreen.tsx`](../../web-app/src/features/auth/LoginScreen.tsx)
- [`features/public/AppDownloadLanding.tsx`](../../web-app/src/features/public/AppDownloadLanding.tsx)
- [`features/settings/LanguageSelector.tsx`](../../web-app/src/features/settings/LanguageSelector.tsx)

## Notas

- `LanguageSelector` com `variant="compact"` + `data-testid` `login-locale-pt` / `login-locale-en`.
- Chaves novas em `auth.json` (pt/en).
- Secção I18N «Onde mudar idioma»: **Login + Definições**.
...
```bash
rg "[À-ú]" web-app/src/features/auth/LoginScreen.tsx web-app/src/features/public/AppDownloadLanding.tsx web-app/src/features/settings/LanguageSelector.tsx
```
...
```
feat(i18n): login-landing-locale
```
