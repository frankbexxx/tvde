...
| **L-06** | Bloqueio de **veículo** não conforme | Art. 14.º n.º 2; PF3D | `PARCIAL` | `GAP FUNCIONAL` | `BLOCKER M2` | Gates viatura **ON** em PROD (G-KYC-P0-04 CLOSED 2026-09-09); IMT/processo oficial e docs pessoais **ainda** abertos |
...
| **L-12** | Retenção **2 anos** reclamações / processo | Art. 19.º n.º 3 | `IMPLEMENTADO` | `SEM GAP` | `FECHADO` | **`FECHADO 2026-09-24`**: retenção `submitted_at + 2 anos civis` já existia. Anexos: tabela `complaint_attachments`, máx. **5**, PDF/JPEG/PNG, **5 MB**, autor + Admin, download autenticado (`Content-Disposition: attachment`), storage `UPLOAD_DIR` no disco persistente, history `attachment_added`. **Não implementado** (não reabre): delete, antivírus, object storage, purge físico. Um purge futuro da complaint tem de apagar a row e o ficheiro. L-25 separado |
| **L-13** | Comissão plataforma **≤ 25% sem IVA** | Art. 15.º n.º 3 | `PARCIAL` | `GAP LEGAL/PROCESSO` | `NECESSÁRIO M2` | **PARCIAL — guard implementado, validação jurídico-fiscal pendente.** Implementado: percentagem configurada **0–25%** inclusive rejeitada fora do intervalo (sem cap silencioso) em `accept_trip`, `accept_offer` e `complete_trip` **antes da captura**; base actual `final − portagens`, pet incluído, piloto **15%** (A1-D01). Por validar: base legal exacta **sem IVA**, tratamento fiscal do IVA, impacto dessa leitura no cálculo. **Não CLOSED** |
| **L-14** | Demonstração de **cálculo** (fórmula / fatores) antes e durante | Art. 15.º n.º 4 al. a); 19.º | `PARCIAL` | `GAP COPY/UX` | `NECESSÁRIO M2` | **IMPLEMENTADO NA APP / PARCIAL.** Após o create, na viagem activa e no histórico, o passageiro vê a fórmula a partir de `price_breakdown` (base, €/km, €/min, mínimo e ajuste ao mínimo se > 0). Sem cálculo no frontend e sem chamadas OSRM/HERE extra. Pet, portagens e total mantêm-se à parte; a taxa L-15 também. Fora: confirmação pré-POST sem preço oficial, factura **L-16**, base IVA **L-13**. **Não CLOSED** |
| **L-15** | Mostrar **taxa de intermediação** ao utilizador | Art. 15.º n.º 4 / n.º 8 | `PARCIAL` | `GAP LEGAL/PROCESSO` | `NECESSÁRIO M2` | **IMPLEMENTADO NA APP / PARCIAL GLOBAL.** Novas viagens gravam `intermediation_rate_percent` = **15%** e o settlement usa esse snapshot; o passageiro vê a mesma taxa na estimativa, na viagem activa e no histórico. Trips legacy `NULL` mantêm `driver.commission_percent` e não são recalculadas. Fora: factura electrónica (**L-16**, art. 15.º n.º 8) e base sem IVA (**L-13**). A fórmula L-14 é outra linha, lida do snapshot, e não entra nesta taxa. **Não CLOSED** |
...
| `NECESSÁRIO M2` | **11** (L-09, L-11, L-13…L-18, L-23, L-24, L-29) — **L-12 e L-25 removidos** 2026-09-24 |
| `DEPENDE PARECER` | **5** (L-07, L-08, L-27, L-28, L-30) |
| `FECHADO` | **3** (L-26 ex-blocker; L-25 operacional 2026-09-24; L-12 anexos 2026-09-24) |
