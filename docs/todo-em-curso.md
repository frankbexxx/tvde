# Em curso — quadro operacional

Vista única para saber sempre o que está em execução, no mesmo formato em todas as sessões.

---

## Agora (foco único)

- [ ] **Aguardar** respostas do Manuel (`docs/partner/MANUEL_DRIVER_QA_2026-04-29.md`).
- [ ] **Aguardar** decisão «dois destinos por dia» + regras finas de categorias (pós-Manuel).
- [x] Linha rotacional no topo (`AppHeaderBar`, hints estáticos — v1).
- [x] Spec menu motorista: `docs/product/DRIVER_MENU_SPEC.md`.
- [x] Categorias: copy no menu + label PT; integração GET/PATCH já existente — evolução depende de regras de negócio.

---

## Hoje (até fechar a sessão)

- [x] Smoke pós-merge (rating, rejeitar, navegação).
- [x] E2E/CI estabilizado (tipagem Playwright, rate-limit, motorista offline).
- [x] Semântica botões + faixa 35/5/60 + hora no header (#202).
- [x] Conta vs Configurações (#203).
- [ ] Perguntas ao Manuel — quando houver respostas, actualizar o ficheiro de QA.

---

## Amanhã (próxima sessão útil)

- [ ] Menu motorista — ecrã ou link **detalhe de viagem** — ver `docs/product/DRIVER_MENU_SPEC.md` (histórico no menu: percurso + «Mostrar mais»).
- [ ] Categorias + «dois destinos por dia» — contrato de dados após Manuel.
- [ ] Linha rotacional v2 (mais mensagens ou dados internos da app, ainda sem APIs externas).
- [ ] Planeamento de login social (Google e afins) como onda própria.

---

## Backlog (não bloquear agora)

- Theming/polish amplo de superfície e iconografia final.
- Refactors estruturais sem impacto directo em operação.

---

_Última revisão: 2026-04-30_
