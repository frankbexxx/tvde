...
- **L-SEC-12 = CLOSED** (consume atómico). OTP em deployed continua `503`. Reactivar exige provider SMS + smoke; esta linha não liga SMS.  
...
- **Estado 2026-09-25:** **PARTIAL**. No Oppo: login utilizável em retrato; localização foreground com «Enquanto uso a app»; mapa visível (MapTiler 403 em `https://localhost`, fallback raster). OAuth Capacitor = **PARTIAL / WAITING EXTERNAL CONFIG**: no Oppo o botão abre o selector Google da app e o regresso é à app, sem loop; a conta seleccionada falha com `[16] Account reauth failed` e não há id token. Falta um cliente OAuth Android no mesmo projecto Google do cliente web, package `pt.vamula.app`, SHA-1 de debug `EC:02:84:8C:1E:AE:1D:AE:8C:D5:87:74:77:83:F7:76:19:71:30:A1` (SHA-256 `07:95:6C:38:52:24:12:C0:54:F8:73:52:44:91:A3:A6:87:53:DE:94:CC:92:52:F9:77:AC:A5:33:F7:BA:FC:30` se a consola pedir). O client id Android não entra na app; `initialize` continua a usar o client id web. Ainda aberto: GPS background, push, Waze/Maps, file picker, Play Store.  
