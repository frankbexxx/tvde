...
**Passenger é capacidade base; `User.role` continua a ser o papel elevado.** Decisão de 2026-09-26, fase 1 só no linking Google. Não há tabela de papéis nem mudança de schema. O telefone continua UNIQUE. Um Google verificado liga-se à conta que já tem esse email, sem criar outra pessoa e sem mudar o `role`. Se o telefone já existe e o email não confirma a mesma conta, não há auto-link: a API pede a palavra-passe (`existing_account_link_required`) ou recusa o conflito (`existing_account_link_conflict`). Não se fundem emails nem subs diferentes. A vista Passageiro para Admin e Partner fica para a fase 2. A capacidade multi-role **não** está CLOSED. A conta Passageiro extra criada no smoke de produção mantém-se até uma limpeza governada: o `oauth_google_sub` só pode existir numa conta.
