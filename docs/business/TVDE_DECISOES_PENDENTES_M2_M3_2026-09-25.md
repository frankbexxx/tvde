...
**S-MOB-02 = PARTIAL** (2026-09-26). No mesmo Oppo: o login cabe em retrato, a localização foreground obtém posição, e o mapa desenha ruas. OAuth Capacitor = **CLOSED**. O Android Credential Manager devolve o nonce fornecido; o cliente passa SHA-256(rawNonce) ao Google e o backend valida contra rawNonce. O plugin é `@capawesome/capacitor-google-sign-in` (Credential Manager), sem custom scheme. O fluxo web (authorization code + redirect allowlist + `state`) mantém-se. Google Passenger Onboarding = **CLOSED**: no Oppo o selector abre, a conta nova fica em `google_onboarding_required` sem sessão, confirma nome, telemóvel +351 e termos, passa a `active` e entra no PassengerDashboard. Smoke feito contra a API de staging (`a63993c`); a PROD não foi alterada. O client id passado a `initialize` continua a ser o client id web. Waze/Maps = **CLOSED** e file picker = **CLOSED** (PR #669, Oppo, 2026-09-26). Push, GPS background e Play Store continuam abertos.

**Passenger is a base capability for any active User.** Decisão de 2026-09-26. Não há tabela de papéis, `roles[]` nem mudança de schema ou de JWT (`sub`, `role`, `token_version`). O telefone continua UNIQUE. Qualquer `User` activo usa o contexto Passageiro; o `role` singular continua a ser o papel elevado. Um motorista não recebe, não aceita e não é atribuído à própria viagem. Passageiro, motorista e parceiro activos ligam o Google quando o email verificado já é o da conta e não há conflito de `sub`. **First-time Google linking for admin/super_admin requires existing-account password confirmation, even when the verified Google email matches.** Depois de o `oauth_google_sub` estar gravado, o login Google seguinte entra sem pedir a palavra-passe outra vez e sem mudar o `role`. Se o telefone já existe e o email não confirma a mesma conta, não há auto-link: a API pede a palavra-passe (`existing_account_link_required`) ou recusa o conflito (`existing_account_link_conflict`). Não se fundem emails nem subs diferentes. GOOG-LINK = **CLOSED**. A capacidade multi-role está **CLOSED**: o selector de contexto não altera `User.role`. A conta Passageiro extra do smoke de produção foi limpa: o `oauth_google_sub` ficou só na conta real e a conta de smoke ficou bloqueada, sem apagar o histórico legal. cleanup smoke account = **CLOSED**.
