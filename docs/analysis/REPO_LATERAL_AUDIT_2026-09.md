...
**P2/P3:** **não recontados** nesta reconciliação. Explicitamente: **L-SEC-09** e **L-SEC-19** → **CLOSED**; **L-SEC-16** → **CLOSED** (driver upload alinhado a viaturas); **L-SEC-15** → **CLOSED** (interaction log Session própria); **L-SEC-13** → **CLOSED** (`token_version`); **L-SEC-10** → **CLOSED** (WS Bearer-only, sem `?token=`); **L-SEC-14** → **PARTIAL** (**14A** CLOSED partner trips cap; **14B** OPEN redispatch; **14C** ACCEPTED DEBT piloto); **L-DOC-03** parcialmente aliviado (env no índice via L-DOC-01). Restante P2/P3 requer nova revisão.
...
| **L-SEC-14** | P2 → **PARTIAL** | SQL | Umbrella. Split: **14A** partner trip list (CLOSED nesta PR), **14B** redispatch N+1 (OPEN), **14C** admin/matching piloto (ACCEPTED DEBT). | ver sub-itens | — | Não fechar umbrella até 14B |
...
| **L-SEC-14B** | P3 | Redispatch | N+1: all `requested` trips → offers por trip; scan drivers+loc. Cron ~1/min. | `offer_dispatch.redispatch_expired_trips` | Custo interno cron | Batch/IN + SQL agg — PR separada |
...
**Segurança:** limites in-process, JWT após password change (CLOSED L-SEC-13), WS query-token (CLOSED L-SEC-10), listagens (L-SEC-14 **PARTIAL**: 14A CLOSED partner cap; 14B OPEN redispatch; 14C ACCEPTED DEBT), `log_interaction` commit (CLOSED L-SEC-15), upload driver allowlist (CLOSED L-SEC-16), OTP API sem canal de entrega. Cron query secret CLOSED (L-SEC-09/19).
