...
**P2/P3:** **não recontados** nesta reconciliação. Explicitamente: **L-SEC-09** e **L-SEC-19** → **CLOSED**; **L-SEC-16** → **CLOSED** (driver upload alinhado a viaturas); **L-SEC-15** → **CLOSED** (interaction log Session própria); **L-SEC-13** → **CLOSED** (`token_version`); **L-SEC-10** → **CLOSED** (WS Bearer-only, sem `?token=`); **L-SEC-11** → **ACCEPTED DEBT** (in-memory + single-worker guard); **L-SEC-12** → **ACCEPTED DEBT** (P3 enquanto OTP não é entregue; **MUST FIX BEFORE SMS**); **L-SEC-17** → **CLOSED** (OTP `503` em deployed); **L-SEC-14** → **PARTIAL / ACCEPTED RESIDUAL** (**14A**/**14B** CLOSED; **14C** ACCEPTED DEBT piloto); **L-DOC-03** parcialmente aliviado (env no índice via L-DOC-01). Restante P2/P3 requer nova revisão.
...
| **L-SEC-11** | P2 arquitectura / **P3** efectivo piloto → **ACCEPTED DEBT** | Rate limit | Limites OTP/login/Google/`request_trip` **por processo em memória**. PROD actual 1×1 → limites ≈ nominais. Scale-out (W×H) **reabre**. Guard: fail-fast se `WEB_CONCURRENCY>1` / `--workers>1` em deployed. OTP *request* ainda chaveia IP via XFF (dívida residual de chave, não de multi-worker). | #646; `in_memory_rate_limit_guard.py`; `auth_rate_limit.py`; `api/rate_limit.py`; `PREPARACAO_RENDER.md` | Multiplicação se scale sem store | Shared store **antes** multi-worker/multi-instance; manter 1 instance + 1 worker |
| **L-SEC-12** | P2 código / **P3** efectivo enquanto OTP inactivo → **ACCEPTED DEBT** | OTP | Verify sem `FOR UPDATE` e sem `UPDATE` condicionado a `consumed_at IS NULL`. Sem contador persistente. Código **não é entregue** (L-SEC-17 bloqueia deployed). **Não CLOSED.** **MUST FIX BEFORE** activar SMS OTP. Preferido: consume atómico (`consumed_at IS NULL` e `expires_at > now`). Contador persistente só quando houver SMS. | `auth.py` `verify_otp`; `otp.py` | Race de double-use se o código for conhecido | Atomic consume antes de ligar SMS |
...
| **L-SEC-14** | P2 → **PARTIAL / ACCEPTED RESIDUAL** | SQL | Umbrella. **14A** CLOSED · **14B** CLOSED · **14C** ACCEPTED DEBT (admin/matching piloto). | ver sub-itens | Residual 14C + redispatch driver-pool/pending-check | Não fechar umbrella enquanto 14C aceite |
| **L-SEC-14A** | P3 → **CLOSED** | Partner | ~~`GET /partner/trips` sem bound.~~ Cap 500 newest; export histórico sem cap. | #644; `partner_queries.PARTNER_TRIP_LIST_LIMIT`; `test_l_sec_14a_partner_trips_cap.py` | — | Fechado |
| **L-SEC-14B** | P3 → **CLOSED** | Redispatch | ~~N+1 `requested`→offers por trip.~~ **Mitigado:** bulk `TripOffer WHERE trip_id IN (...)`. Residual (dívida separada): driver pool por trip; `_has_active_pending_offer` por candidato. | #645; `offer_dispatch.redispatch_expired_trips`; `test_l_sec_14b_redispatch_offers_bulk.py` | — | Fechado |
| **L-SEC-14C** | P3 **ACCEPTED DEBT** | Admin / matching | Admin partners/drivers/pending unbounded mas staff-only + cardinalidade piloto; matching pool approved+available+GPS + top_n=5 (find-driver CLOSED). | `admin.py`; `offer_dispatch.create_offers_for_trip` | Escala futura | Revisitar com crescimento |
| **L-SEC-15** | P2 → **CLOSED** | TX | ~~`log_interaction` faz `db.commit()`/`rollback()` na Session do pedido.~~ **Mitigado:** escrita via `SessionLocal` própria; `db` do caller só por compatibilidade de assinatura (não usado). | #640; `interaction_logging.py`; `test_l_sec_15_interaction_log_own_session.py` | — | Fechado |
...
| **L-SEC-17** | P2 → **CLOSED** | OTP | ~~API OTP pública sem SMS.~~ **Mitigado em deployed:** `POST /auth/otp/request` e `/verify` devolvem **503** `otp_auth_unavailable` antes de gerar, gravar ou emitir JWT. Código OTP **permanece** para o carril C1. Non-prod inalterado. Reactivar em deployed exige SMS real testado **e** L-SEC-12 resolvido. | `auth.py` `_reject_otp_when_deployed`; `test_l_sec_17_disable_prod_otp.py` | — | Fechado para PROD actual |
...
**Segurança:** limites in-process (L-SEC-11 **ACCEPTED DEBT** + single-worker guard), JWT após password change (CLOSED L-SEC-13), WS query-token (CLOSED L-SEC-10), listagens (L-SEC-14 **PARTIAL / ACCEPTED RESIDUAL**: 14A/14B CLOSED; 14C ACCEPTED DEBT), `log_interaction` commit (CLOSED L-SEC-15), upload driver allowlist (CLOSED L-SEC-16), OTP deployed **503** (CLOSED L-SEC-17; L-SEC-12 **ACCEPTED DEBT** / MUST FIX BEFORE SMS). Cron query secret CLOSED (L-SEC-09/19).
