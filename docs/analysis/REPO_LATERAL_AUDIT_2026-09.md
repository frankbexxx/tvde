...
**P2/P3:** **não recontados** nesta reconciliação. Explicitamente: **L-SEC-09** e **L-SEC-19** → **CLOSED**; **L-SEC-16** → **CLOSED** (driver upload alinhado a viaturas); **L-SEC-15** → **CLOSED** (interaction log Session própria); **L-SEC-13** → **CLOSED** (`token_version`); **L-SEC-10** → **CLOSED** (WS Bearer-only, sem `?token=`); **L-SEC-11** → **ACCEPTED DEBT** (in-memory + single-worker guard); **L-SEC-12** → **CLOSED** (consume atómico; SMS/provider continua por implementar; OTP em deployed continua 503); **L-SEC-17** → **CLOSED** (OTP `503` em deployed); **L-SEC-14** → **PARTIAL / ACCEPTED RESIDUAL** (**14A**/**14B** CLOSED; **14C** ACCEPTED DEBT piloto); **L-DOC-03** parcialmente aliviado (env no índice via L-DOC-01). Restante P2/P3 requer nova revisão.
...
| **L-SEC-12** | P2 → **CLOSED** | OTP | Verify faz `UPDATE` com `consumed_at IS NULL` e `expires_at > now`. Um verify concorrente altera zero linhas e não recebe JWT. Sem contador persistente (só quando houver SMS). OTP em deployed continua **503**. Activar no futuro exige provider SMS + smoke. | `auth.py` `claim_unconsumed_otp`; `test_l_sec_12_atomic_otp_consume.py` | — | Fechado |
