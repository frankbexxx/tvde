...
| **L-SEC-13** | P2 → **CLOSED** | Sessions | ~~Change password não invalida JWTs.~~ **Mitigado:** `users.token_version` + claim JWT; bump em `/auth/me/password`; HTTP/WS rejeitam mismatch. Legacy sem claim ≡ 0. FE faz logout pós-change. | #PR; migration `e3f4a5b6c7d8`; `security.py` / `deps.py` / `ws.py`; `test_l_sec_13_token_version.py` | — | Fechado |
