...
**P2/P3:** **não recontados** nesta reconciliação. Explicitamente: **L-SEC-09** e **L-SEC-19** → **CLOSED**; **L-SEC-16** → **CLOSED** (driver upload alinhado a viaturas); **L-SEC-15** → **CLOSED** (interaction log Session própria); **L-SEC-13** → **CLOSED** (`token_version`); **L-SEC-10** → **CLOSED** (WS Bearer-only, sem `?token=`); **L-SEC-14** → **PARTIAL / ACCEPTED RESIDUAL** (**14A**/**14B** CLOSED; **14C** ACCEPTED DEBT piloto); **L-DOC-03** parcialmente aliviado (env no índice via L-DOC-01). Restante P2/P3 requer nova revisão.
