...
| **L-SEC-10** | P2 → **CLOSED** | WebSocket | ~~Token JWT aceite em `?token=` (além de `Authorization`).~~ **Mitigado:** fallback query removido; auth WS **apenas** `Authorization: Bearer`. FE PROD não usa WS (polling HTTP). | #TBD; `ws.py` / `admin_ws.py`; `test_l_sec_10_ws_bearer_only.py` | — | Fechado |
