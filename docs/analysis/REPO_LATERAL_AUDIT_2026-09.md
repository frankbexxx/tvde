...
| **L-SEC-15** | P2 → **CLOSED** | TX | ~~`log_interaction` faz `db.commit()`/`rollback()` na Session do pedido.~~ **Mitigado:** escrita via `SessionLocal` própria; `db` do caller só por compatibilidade de assinatura (não usado). | #PR; `interaction_logging.py`; `test_l_sec_15_interaction_log_own_session.py` | — | Fechado |
