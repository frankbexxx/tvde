...
| **L-SEC-14B** | P3 → **CLOSED** | Redispatch | ~~N+1 `requested`→offers por trip.~~ **Mitigado:** bulk `TripOffer WHERE trip_id IN (...)`. Residual (dívida separada): driver pool por trip; `_has_active_pending_offer` por candidato. | #TBD; `offer_dispatch.redispatch_expired_trips`; `test_l_sec_14b_redispatch_offers_bulk.py` | — | Fechado |
