...
- **`ROTACIONAL_V3_FETCH_URL`** (opcional): URL que devolve **JSON array** no mesmo formato; o batch **`GET /cron/jobs`** (header `X-Cron-Secret`) refresca a tabela `rotacional_external_cache`. Se vazio, o passo é no-op. **`ROTACIONAL_V3_FETCH_TIMEOUT_SECONDS`** (opcional, default ~8).
