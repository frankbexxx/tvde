...
| `ADMIN_PHONE` | Opcional | Opcional | Owner phone for **seed/backfill** (`is_owner_phone`) only. **Does not** grant roles at login and **does not** protect accounts in admin APIs — staff protection is role-based (`admin` / `super_admin`). |
