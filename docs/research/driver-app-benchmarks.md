...
Referências externas e notas de pesquisa (não são especificação implementável por si só). Última revisão: **2026-05-03**.
...
- **Portagens:** dois percursos (rápido vs barato); preferência em **Conta** + **Definições**; recalcular **sempre** se o percurso mudar (cortes, etc.); só pagamentos que permitam **extras**; UX dinâmica se a viagem puder incluir portagens — spec técnica mínima: [`PORTAGENS_SPEC.md`](../product/PORTAGENS_SPEC.md).
- **Ecrã:** persistir visibilidade da app para o motorista ver o estado (alinhar com wake lock / política “quando”).
- **Wireframes (4×4):** ecrã principal (mapa em fundo, GO centrado, HOME\|EARNINGS\|INBOX\|MENU); ecrã mapa (ESTATUTO, DIAMOND, **LUPA** = modo destino; barra inferior tipo ops). Especificação alargada e imagem: [`DRIVER_HOME_TOP3_MANEL.md`](../product/DRIVER_HOME_TOP3_MANEL.md) **§9**.
