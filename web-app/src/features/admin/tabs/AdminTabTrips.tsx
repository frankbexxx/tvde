...
import type { AdminAuditTrailItem, TripActiveItem, TripDetailAdmin } from '../../../api/admin'
...
export type AdminTabTripsProps = {
  ...
  tripAuditRows: AdminAuditTrailItem[] | undefined
  tripAuditLoading: boolean
  tripAuditError: string | null
  fetchTripAuditTrail: (tripId: string) => void | Promise<void>
}
...
export function AdminTabTrips(props: AdminTabTripsProps) {
  ...
  const stripeLinks = (pi: unknown) => {
    if (typeof pi !== 'string' || !pi) return null
    return stripePaymentIntentDashboardUrls(pi)
  }
          ...
          <div className="rounded-2xl border-2 border-primary/40 bg-primary/5 px-4 py-4 shadow-card space-y-3">
            ...
            {tripDetailLoading ? (
              ...
              <div className="space-y-2 rounded-xl border border-border bg-background/80 p-3">
                <AdminTripDetailSupportFields tripDetail={tripDetail} />
                {(() => {
                  const urls = stripeLinks(tripDetail.stripe_payment_intent_id)
                  if (urls) {
                    return (
                  ...
                  if (typeof tripDetail.stripe_payment_intent_id === 'string' && tripDetail.stripe_payment_intent_id) {
                    return (
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  {tripDetail.status === 'requested' && (
                    <button
                      ...
                      title={ADMIN_ASSIGN_RECOVERY_TITLE}
                ...
                <AdminTripAuditTimeline
                  rows={tripAuditRows}
                  loading={tripAuditLoading}
                  error={tripAuditError}
                  onRetry={() => void fetchTripAuditTrail(selectedTripId)}
                />
                {selectedTripId ? (
                      ...
                      {selectedTripId === t.trip_id && (
                        <div className="mt-3 pt-3 border-t border-border space-y-2">
                          {tripDetailLoading ? (
                            ...
                            <>
                              ...
                              {(() => {
                                ...
                                if (
                                  typeof tripDetail.stripe_payment_intent_id === 'string' &&
                                  tripDetail.stripe_payment_intent_id
                                ) {
                                  return (
                                    <p className="text-xs text-muted-foreground">
                          ...
                          <AdminTripAuditTimeline
                            ...
                            loading={tripAuditLoading}
                            error={tripAuditError}
                            onRetry={() => void fetchTripAuditTrail(t.trip_id)}
                          />
                          <AdminTripPaymentOpsNotePanel
                            tripId={t.trip_id}
                            tripDetail={tripDetail}
                    ...
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        ...
                        <p className="text-xs text-foreground/70">
                          ...
                          {h.payment_status
                            ? ` · Pagamento: ${adminPaymentStatusLabel(h.payment_status)}`
                            : null}
                        </p>
                      </div>
                      <button
                    ...
                    {selectedTripId === h.trip_id && (
                      <div className="mt-3 pt-3 border-t border-border space-y-2">
                        ...
                        <AdminTripAuditTimeline
                          ...
                          loading={tripAuditLoading}
                          error={tripAuditError}
                          onRetry={() => void fetchTripAuditTrail(h.trip_id)}
                        />
                        <AdminTripPaymentOpsNotePanel
                          tripId={h.trip_id}
                          tripDetail={tripDetail}
