...
export type AdminTabOpsProps = {
  ...
  handleDrivingRestSet: () => void | Promise<void>
  handleDrivingRestClear: () => void | Promise<void>
  drivingRestDriverId: string
  drivingRestUntilLocal: string
  setDrivingRestDriverId: Dispatch<SetStateAction<string>>
  setDrivingRestUntilLocal: Dispatch<SetStateAction<string>>
...
export function AdminTabOps(props: AdminTabOpsProps) {
  const {
    ...
    handleRecoverDriver,
    handleDrivingRestSet,
    handleDrivingRestClear,
    drivingRestDriverId,
    drivingRestUntilLocal,
    setDrivingRestDriverId,
    setDrivingRestUntilLocal,
    handleRunCronNow,
    ...
    <>
      <section className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="pt-4 border-t border-border space-y-3">
            <p className="text-sm font-medium text-foreground">Repouso TVDE (override administrativo)</p>
            <p className="text-xs text-muted-foreground leading-snug">
              Define ou limpa <span className="font-mono">driving_rest_until</span> em UTC. Acção com{' '}
              <strong className="text-foreground">auditoria SP-F</strong> — só conta{' '}
              <span className="font-mono">admin</span>; exposição a partner fica fora até haver fluxo não-legal
              específico.
            </p>
            <label className="block text-xs text-foreground/80" htmlFor="driving-rest-driver">
              User ID do motorista (UUID)
            </label>
            <input
              id="driving-rest-driver"
              type="text"
              value={drivingRestDriverId}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setDrivingRestDriverId(e.target.value)}
              placeholder="user_id do motorista"
              className="w-full px-3 py-2 border border-border rounded-lg text-sm font-mono bg-background"
            />
            <label className="block text-xs text-foreground/80" htmlFor="driving-rest-until">
              Repouso até (hora local deste dispositivo)
            </label>
            <input
              id="driving-rest-until"
              type="datetime-local"
              value={drivingRestUntilLocal}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setDrivingRestUntilLocal(e.target.value)}
              className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background"
            />
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={handleDrivingRestSet}
                disabled={!!opsLoading || !drivingRestDriverId.trim()}
                className="px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium disabled:opacity-50 touch-manipulation min-h-11"
              >
                {opsLoading === 'driving-rest' ? 'A aplicar…' : 'Definir repouso até…'}
              </button>
              <button
                type="button"
                onClick={handleDrivingRestClear}
                disabled={!!opsLoading || !drivingRestDriverId.trim()}
                className="px-4 py-2 border border-border bg-card rounded-lg text-sm font-medium disabled:opacity-50 touch-manipulation min-h-11"
              >
                Limpar repouso
              </button>
            </div>
